...
class BookingRegistrationFrame(customtkinter.CTkScrollableFrame):
    ...
    def fetch_vehicles(self):
        ...
        try:
            print("Sudina")
            print(self.parentPage.get_customer_credential())
            response = APIFunctions.VehicleAPI().get_all(auth = self.parentPage.get_customer_credential())
            ...
            self.vehicle_combo.set(vehicle_options[0])
        except:
    ...
    def submit_booking(self):
        ...
        if True and not vehicle_selection or not start_location or not end_location or not start_time or not end_time:
            messagebox.showerror("Error", "All fields are required.")
            return

        print("Krishan", self.parentPage.userDetail["user"]["id"])
        # Prepare data for POST request
        booking_data = {
            "vehicle_id": int(vehicle_selection),
            "start_location": start_location,
            "end_location": end_location,
            "start_time": start_time,
            "end_time": end_time,
            "user_id": int(self.parentPage.userDetail["user"]["id"])
        }
        print(booking_data)
        # Make POST request to create the booking
        
        api = APIFunctions.BookingAPI()
        response = api.create(booking_data, auth=self.parentPage.get_customer_credential())
        print(response)      
...
class ViewCustomerBookingsFrame(customtkinter.CTkScrollableFrame):
    def __init__(self, parentPage, **kwargs):
        self.parentPage = parentPage 
        super().__init__(parentPage, **kwargs)    


        self.logo_label = customtkinter.CTkLabel(self, text="View Your Bookings", font=customtkinter.CTkFont(size=20, weight="bold"))
    ...
    def fetch_bookings(self):
        ...
        """
        response = APIFunctions.BookingAPI().get_all(auth = self.parentPage.get_customer_credential())
        if response.status_code != 200:
            return messagebox.showerror('Error', response.json_data)
        ...
        for booking in bookings:
            my_user_id = self.parentPage.userDetail["user"]["id"]
            if not (booking["vehicle"] or booking["user"]):
                continue
            booking_user_id = booking["user"]["id"]
            if(my_user_id != booking_user_id):
                continue
